...
impl fmt::Display for Bytes {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
			...
			write!(f, "{:7.2}T", self.0 as f32 / (1024. * 1024. * 1024. * 1024.))
...
impl<T> fmt::Display for Threshold<T> where T: fmt::Display + Ord {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
fn main() {
	if !cfg!(target_os = "linux") {
		writeln!(io::stderr(), "Hitome only works by reading Linux-specific /proc interfaces, sorry.").unwrap();
                return;
	}
